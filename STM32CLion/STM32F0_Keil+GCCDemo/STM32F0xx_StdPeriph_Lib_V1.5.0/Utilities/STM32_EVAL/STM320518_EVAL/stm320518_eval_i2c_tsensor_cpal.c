...
static uint32_t LM75_Status(void) {
    LM75_DevStructure.pCPAL_TransferTx = &LM75_TXTransfer;
    LM75_DevStructure.pCPAL_TransferTx->wAddr1 = (uint32_t)
    LM75_ADDR;
...
uint16_t LM75_ReadReg(uint8_t RegName) {
    ...
    LM75_DevStructure.pCPAL_TransferRx->wAddr1 = (uint32_t)
    LM75_ADDR;
    LM75_DevStructure.pCPAL_TransferRx->wAddr2 = (uint32_t)
    RegName;

    /* Read Operation */
    if (CPAL_I2C_Read(&LM75_DevStructure) == CPAL_PASS) {
        while ((LM75_DevStructure.CPAL_State != CPAL_STATE_READY) &&
               (LM75_DevStructure.CPAL_State != CPAL_STATE_ERROR)) {}
    }
...
uint8_t LM75_WriteReg(uint8_t RegName, uint16_t RegValue) {
    ...
    LM75_DevStructure.pCPAL_TransferTx->wAddr1 = (uint32_t)
    LM75_ADDR;
    LM75_DevStructure.pCPAL_TransferTx->wAddr2 = (uint32_t)
    RegName;
