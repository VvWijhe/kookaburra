...
LCD_DrawChar(Line, Column, &LCD_Currentfonts
->table[
Ascii *LCD_Currentfonts
->Height]);
}
...
LCD_DrawLine(Xpos, Ypos, Width,
LCD_DIR_HORIZONTAL);
LCD_DrawLine((Xpos
+ Height), Ypos, Width, LCD_DIR_HORIZONTAL);

LCD_DrawLine(Xpos, Ypos, Height,
LCD_DIR_VERTICAL);
LCD_DrawLine(Xpos, (Ypos
- Width + 1), Height, LCD_DIR_VERTICAL);
}
...
{
LCD_DrawLine(Xpos
- CurX, Ypos + CurY, 2*CurY, LCD_DIR_HORIZONTAL);
LCD_DrawLine(Xpos
+ CurX, Ypos + CurY, 2*CurY, LCD_DIR_HORIZONTAL);
}
...
{
LCD_DrawLine(Xpos
- CurY, Ypos + CurX, 2*CurX, LCD_DIR_HORIZONTAL);
LCD_DrawLine(Xpos
+ CurY, Ypos + CurX, 2*CurX, LCD_DIR_HORIZONTAL);
}
if (D < 0)
...
{
num -=
den;               /* Calculate the new numerator value */
x +=
xinc1;               /* Change the x as appropriate */
y +=
yinc1;               /* Change the y as appropriate */
}
x +=
xinc2;                 /* Change the x as appropriate */
y +=
yinc2;                 /* Change the y as appropriate */
}
}
...
while(
SPI_I2S_GetFlagStatus(LCD_SPI, SPI_I2S_FLAG_TXE
) != SET)
{
}
/* Send the byte */
...
while(
SPI_I2S_GetFlagStatus(LCD_SPI, SPI_I2S_FLAG_RXNE
) != SET)
{
}
...
while(
SPI_I2S_GetFlagStatus(LCD_SPI, SPI_I2S_FLAG_BSY
) != RESET)
{
}
}
...
SPI_SendData8(LCD_SPI, LCD_RegValue
>>8);
...
SPI_SendData8(LCD_SPI, RGB_Code
>>8);
