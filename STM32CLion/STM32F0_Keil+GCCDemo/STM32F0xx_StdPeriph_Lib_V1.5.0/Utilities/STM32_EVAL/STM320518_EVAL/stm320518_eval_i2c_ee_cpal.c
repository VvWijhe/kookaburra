...
typedef struct {
    __IO uint32_t
    sEEDataNum;             /* The number of data that will be written in next transfer */
    ...
    __IO uint16_t
    sEENumOfPage;           /* The number of page that will be written  */

    __IO uint8_t
    sEENumOfSingle;          /* The number of single data that will be written */

    __IO uint8_t
    sEENextWrite;            /* This member indicates there is remaining transfers  */
    ...
    if (sEEInitStruct->sEEState == sEE_STATE_IDLE) {
        ...
        if (NumByteToWrite == 1) {
            ...
            sEE_WriteStructures[sEEInitStruct->sEE_CPALStructure->CPAL_Dev]->sEENumOfSingle;
                    ...
                    /* update number of data to write */
                    sEE_WriteStructures[sEEInitStruct->sEE_CPALStructure->CPAL_Dev]->sEEDataNum = (uint32_t)(
                            (uint16_t) sEEInitStruct->sEEPageSize);
            ...
            sEE_WriteStructures[sEEInitStruct->sEE_CPALStructure->CPAL_Dev]->sEEWriteAddr = (uint32_t)(
                    (uint16_t) WriteAddr);
