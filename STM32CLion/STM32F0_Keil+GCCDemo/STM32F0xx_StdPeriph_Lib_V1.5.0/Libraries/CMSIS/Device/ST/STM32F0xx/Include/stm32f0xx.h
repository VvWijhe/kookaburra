...
typedef struct {
    ...
    __IO uint32_t
    RMPCR;        /*!< Remap control register,                                      Address offset: 0xA8 */
} DMA_TypeDef;
...
typedef struct {
    ...
    __IO uint32_t
    IT_LINE_SR[32]; /*!< SYSCFG configuration IT_LINE register,                  Address offset: 0x80 */
...
typedef struct {
    ...
    __IO uint32_t
    WUTR;       /*!< RTC wakeup timer register,(only for STM32F072 devices)    Address offset: 0x14 */
