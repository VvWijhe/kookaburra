...
static void RTC_Time_display(uint8_t
Line,
__IO uint16_t
Color_x,
Table_TypeDef table
...
int main(void) {
    ...
    LCD_DisplayStringLine(LCD_LINE_0, (uint8_t * )
    "  TimeStamp Example " );
...
static Table_TypeDef RTC_Get_Time(uint32_t Secondfraction, RTC_TimeTypeDef *RTC_TimeStructure) {
    ...
    table2.tab[4] = (((uint8_t)(RTC_TimeStructure->RTC_Minutes & 0x0F)) + (uint8_t)
    0x30);
    table2.tab[5] = 0x3A;
...
static Table_TypeDef RTC_Get_Date(RTC_DateTypeDef *RTC_DateStructure) {
    ...
    table2.tab[4] = (((uint8_t)(RTC_DateStructure->RTC_Month & 0x0F)) + (uint8_t)
    0x30);
    table2.tab[5] = 0x2F;
