    ...
    for (; btr;                                    /* Repeat until all data transferred */
        ...
        if ((fp->fptr % SS(fp->fs)) == 0) {            /* On the sector boundary? */
            ...
            if (cc) {                                /* Read maximum contiguous sectors directly */
                ...
                if (disk_read(fp->fs->drv, rbuff, sect, (BYTE) cc) != RES_OK) ABORT(fp->fs, FR_DISK_ERR);
            ...
            if (fp->dsect != sect) {                /* Fill sector buffer with file data */
                if (disk_read(fp->fs->drv, fp->buf, sect, 1) != RES_OK) ABORT(fp->fs, FR_DISK_ERR);
            }
#endif
            fp->dsect = sect;
        }
    ...
    for (; btw;                                    /* Repeat until all data transferred */
        ...
        if ((fp->fptr % SS(fp->fs)) == 0) {            /* On the sector boundary? */
            ...
            if (cc) {                                /* Write maximum contiguous sectors directly */
                if (csect + cc > fp->fs->csize)        /* Clip at cluster boundary */
                    cc = fp->fs->csize - csect;
                if (disk_write(fp->fs->drv, wbuff, sect, (BYTE) cc) != RES_OK) ABORT(fp->fs, FR_DISK_ERR);
#if _FS_TINY
                    ...
                    if (res == FR_OK) {
                        ...
                        if (djo.sclust != djn.sclust &&
                            ...
                            if (!dw) {
                                ...
                                if (res == FR_OK && dir[1] == '.') {
                                    dw = (djn.fs->fs_type == FS_FAT32 && djn.sclust == djn.fs->dirbase) ? 0
                                                                                                        : djn.sclust;
                                    ST_CLUST(dir, dw);
                                    djn.fs->wflag = 1;
                                }
