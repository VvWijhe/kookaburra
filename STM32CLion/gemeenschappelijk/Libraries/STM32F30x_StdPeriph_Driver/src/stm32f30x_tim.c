...
/**
  ...
  * @retval None
  */
void TIM_Break1Config(TIM_TypeDef *TIMx, uint32_t TIM_Break1Polarity,
                      uint8_t TIM_Break1Filter) {   /* Check the parameters */
    assert_param(IS_TIM_LIST4_PERIPH(TIMx));
...
void TIM_ETRConfig(TIM_TypeDef *TIMx, uint16_t TIM_ExtTRGPrescaler,
    ...
    tmpsmcr |= (uint16_t)(
            TIM_ExtTRGPrescaler | (uint16_t)(TIM_ExtTRGPolarity | (uint16_t)(ExtTRGFilter << (uint16_t) 8)));
...
void TIM_EncoderInterfaceConfig(TIM_TypeDef *TIMx, uint16_t TIM_EncoderMode,
    ...
    tmpccmr1 &= ((uint16_t)
    ~TIM_CCMR1_CC1S) &((uint16_t)
    ~TIM_CCMR1_CC2S);
    tmpccmr1 |= TIM_CCMR1_CC1S_0 | TIM_CCMR1_CC2S_0;

    /* Set the TI1 and the TI2 Polarities */
    tmpccer &= ((uint16_t)
    ~TIM_CCER_CC1P) &((uint16_t)
    ~TIM_CCER_CC2P);
