...
void OPAMP_DeInit(uint32_t OPAMP_Selection) {
    ...
    *(__IO
    uint32_t *) (OPAMP_BASE + OPAMP_Selection) = ((uint32_t) 0x00000000);
}
...
void OPAMP_Init(uint32_t OPAMP_Selection, OPAMP_InitTypeDef *OPAMP_InitStruct) {
    ...
    tmpreg = *(__IO
    uint32_t *) (OPAMP_BASE + OPAMP_Selection);
    ...
    *(__IO
    uint32_t *) (OPAMP_BASE + OPAMP_Selection) = tmpreg;
}
...
void OPAMP_PGAConfig(uint32_t OPAMP_Selection, uint32_t OPAMP_PGAGain, uint32_t OPAMP_PGAConnect) {
    ...
    *(__IO
    uint32_t *) (OPAMP_BASE + OPAMP_Selection) &= (uint32_t)(~OPAMP_CSR_PGGAIN);

    /* Set the new configuration */
    *(__IO
    uint32_t *) (OPAMP_BASE + OPAMP_Selection) |= (uint32_t)(OPAMP_PGAGain | OPAMP_PGAConnect);
}
...
void OPAMP_VrefConnectNonInvertingInput(uint32_t OPAMP_Selection, FunctionalState NewState) {
    ...
    if (NewState != DISABLE) {
        ...
        *(__IO
        uint32_t *) (OPAMP_BASE + OPAMP_Selection) |= (uint32_t)(OPAMP_CSR_FORCEVP);
    } else {
...
void OPAMP_VrefConnectADCCmd(uint32_t OPAMP_Selection, FunctionalState NewState) {
    ...
    if (NewState != DISABLE) {
        ...
        *(__IO
        uint32_t *) (OPAMP_BASE + OPAMP_Selection) |= (uint32_t)(OPAMP_CSR_TSTREF);
    } else {
...
void OPAMP_TimerControlledMuxCmd(uint32_t OPAMP_Selection, FunctionalState NewState) {
    ...
    if (NewState != DISABLE) {
        ...
        *(__IO
        uint32_t *) (OPAMP_BASE + OPAMP_Selection) |= (uint32_t)(OPAMP_CSR_TCMEN);
    } else {
...
void OPAMP_Cmd(uint32_t OPAMP_Selection, FunctionalState NewState) {
    ...
    if (NewState != DISABLE) {
        ...
        *(__IO
        uint32_t *) (OPAMP_BASE + OPAMP_Selection) |= (uint32_t)(OPAMP_CSR_OPAMPxEN);
    } else {
...
void OPAMP_OffsetTrimModeSelect(uint32_t OPAMP_Selection, uint32_t OPAMP_Trimming) {
    ...
    /* Reset USERTRIM bit */
    *(__IO
    uint32_t *) (OPAMP_BASE + OPAMP_Selection) &= (~(uint32_t)(OPAMP_CSR_USERTRIM));

    /* Select trimming mode */
    *(__IO
    uint32_t *) (OPAMP_BASE + OPAMP_Selection) |= OPAMP_Trimming;
}
...
void OPAMP_OffsetTrimConfig(uint32_t OPAMP_Selection, uint32_t OPAMP_Input, uint32_t OPAMP_TrimValue) {
    ...
    tmpreg &= ((uint32_t)
    ~(OPAMP_CSR_TRIMMING_MASK << OPAMP_Input));
...
void OPAMP_StartCalibration(uint32_t OPAMP_Selection, FunctionalState NewState) {
    ...
    if (NewState != DISABLE) {
        ...
        *(__IO
        uint32_t *) (OPAMP_BASE + OPAMP_Selection) |= (uint32_t)(OPAMP_CSR_CALON);
    } else {
...
void OPAMP_LockConfig(uint32_t OPAMP_Selection) {
    ...
    *(__IO
    uint32_t *) (OPAMP_BASE + OPAMP_Selection) |= (uint32_t)(OPAMP_CSR_LOCK);
}
