...
uint32_t CPAL_I2C_Init(CPAL_InitTypeDef *pDevInitStruct) {
    ...
    if ((pDevInitStruct->CPAL_State == CPAL_STATE_READY)
        ...
        CPAL_I2C_HAL_ITInit(pDevInitStruct->CPAL_Dev, pDevInitStruct->wCPAL_Options, pDevInitStruct->CPAL_Direction,
                            pDevInitStruct->CPAL_ProgModel);
...
uint32_t CPAL_I2C_DeInit(CPAL_InitTypeDef *pDevInitStruct) {
    ...
    if ((pDevInitStruct->CPAL_State == CPAL_STATE_READY)
        ...
        ---------------------------------------------------------------------------*/
        CPAL_I2C_HAL_ITDeInit(pDevInitStruct->CPAL_Dev, pDevInitStruct->wCPAL_Options, pDevInitStruct->CPAL_Direction,
                              pDevInitStruct->CPAL_ProgModel);
...
uint32_t CPAL_I2C_StructInit(CPAL_InitTypeDef *pDevInitStruct) {
    ...
    pDevInitStruct->pCPAL_TransferTx = (CPAL_TransferTypeDef *) pNULL;         /* Point pCPAL_TransferTx to a Null pointer */
    pDevInitStruct->pCPAL_TransferRx = (CPAL_TransferTypeDef *) pNULL;         /* Point pCPAL_TransferRx to a Null pointer */
    pDevInitStruct->CPAL_State = CPAL_STATE_DISABLED;                  /* Device Disabled */
    pDevInitStruct->wCPAL_DevError = CPAL_I2C_ERR_NONE;                    /* No Device Error */
...
uint32_t CPAL_I2C_IsDeviceReady(CPAL_InitTypeDef *pDevInitStruct) {
    ...
    /* Send Slave address with bit0 reset for write */
    __CPAL_I2C_HAL_SEND((pDevInitStruct->CPAL_Dev),
                        (uint8_t)((pDevInitStruct->pCPAL_TransferTx->wAddr1) & (uint8_t)(~I2C_OAR1_ADD0)));
