...
static uint8_t usbd_audio_DataOut(void *pdev, uint8_t epnum) {
    if (epnum == AUDIO_OUT_EP) {
        ...
        if (IsocOutWrPtr >= (IsocOutBuff + (AUDIO_OUT_PACKET * OUT_PACKET_NUM))) {/* All buffers are full: roll back */
...
static uint8_t usbd_audio_SOF(void *pdev) {
    ...
    if (PlayFlag) {
        ...
        if (IsocOutRdPtr >= (IsocOutBuff + (AUDIO_OUT_PACKET * OUT_PACKET_NUM))) {/* Roll back to the start of buffer */
