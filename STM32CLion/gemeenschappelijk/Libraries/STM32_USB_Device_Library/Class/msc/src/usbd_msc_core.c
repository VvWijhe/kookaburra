...
USBD_MSC_CfgDesc[USB_MSC_CONFIG_DESC_SIZ]
__ALIGN_END =
        {
        ...
        case USB_REQ_TYPE_STANDARD:
            switch (req->bRequest) {
                ...
                case USB_REQ_CLEAR_FEATURE:
                    ...
                    DCD_EP_Flush(pdev, (uint8_t)
                    req->wIndex);

                    /* Re-activate the EP */
                    DCD_EP_Close(pdev, (uint8_t)
                    req->wIndex);
                    ...
                    /* Handle BOT error */
                    MSC_BOT_CplClrFeature(pdev, (uint8_t)
                    req->wIndex);
                    break;
...
*/
uint8_t *USBD_MSC_GetCfgDesc(uint8_t
speed,
uint16_t *length
)
{
